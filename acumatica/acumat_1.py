...
if __name__ == "__main__":
    for (root, dirs, files) in os.walk(zip_folder):
        for filename in files:
            if re.search(r'acumatica(.*?).csv', filename):
#            if filename.endswith("csv") and "acumatica" in filename:
...
for (root, dirs, files) in os.walk(zip_folder):
    for filename in files:
        if "acumatica" in filename:
            os.remove(str(os.path.join(root, filename)))
